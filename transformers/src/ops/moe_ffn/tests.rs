...
fn distinct_moe_branches_survive_codegen() -> TractResult<()> {
    for change_weights in [false, true] {
        ...
        assert_ne!(op, &biased);
        let mut state = op.state(&TurnState::default(), 0)?.unwrap();
        let other = optimized
            .nodes()
            .iter()
            .filter_map(|n| n.op_as::<OptMoeFfn>())
            ...
            .unwrap();
        let error = state
            .eval(&mut TurnState::default(), other, tvec![input.clone().into_tvalue()])
            .unwrap_err();
        assert!(error.to_string().contains("different plan"));
        state.eval(&mut TurnState::default(), &cloned, tvec![input.clone().into_tvalue()])?;
        assert!(
            state
                .eval(
...
#[test]
fn optimized_expert_state_snapshot_is_independent() -> TractResult<()> {
    for packed in [false, true] {
        let mut model = TypedModel::default();
        let x = model.add_source("x", f32::fact([2, 32]))?;
        let wg = model.add_const("wg", Tensor::zero::<f32>(&[2, 32])?)?;
        let weights = tract_ndarray::Array3::from_shape_fn((2, 32, 32), |(e, i, j)| {
            ((e * 19 + i * 7 + j * 3) % 31) as f32 / 100.0 - 0.15
        })
        .into_tensor();
        let mut inputs = tvec!(x, wg);
        for name in ["w1", "w2", "w3"] {
            inputs.push(if packed {
                add_q40_const(&mut model, name, weights.clone())?
            } else {
                model.add_const(name, weights.clone())?
            });
        }
        let op =
            MoeFfn::basic_with_layout(2, "silu", GateMode::SoftmaxTopk, true, ExpertLayout::Linear);
        ...
        model.select_output_outlets(&output)?;
        let model = model.into_optimized()?;
        let op = model.nodes().iter().find_map(|n| n.op_as::<OptMoeFfn>()).unwrap();
        assert_eq!(op.uses_direct_q40(), packed);
        let plan = SimplePlan::new(model)?;
        let input = tract_ndarray::Array2::from_shape_fn((2, 32), |(i, j)| (i + j) as f32 / 32.0)
            .into_tensor();
        let mut state = plan.spawn()?;
        let expected = state.run(tvec!(input.clone().into()))?;
        let frozen = state.freeze();
        let mut left = frozen.unfreeze();
        let mut right = frozen.unfreeze();
        left.run(tvec!(Tensor::zero::<f32>(&[2, 32])?.into()))?;
        for state in [&mut left, &mut right] {
            let actual = state.run(tvec!(input.clone().into()))?;
            actual[0].close_enough(&expected[0], Approximation::Approximate)?;
        }
    }
    Ok(())
