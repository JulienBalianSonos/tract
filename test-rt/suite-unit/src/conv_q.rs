...
impl QConvProblem {
    ...
    fn reference(&self) -> Tensor {
        ...
        let x0 = *self.qp[0].try_as_plain_ram().unwrap().to_scalar::<i32>().unwrap();
        let k0 = *self.qp[2].try_as_plain_ram().unwrap().to_scalar::<i32>().unwrap();
        let y0 = *self.qp[4].try_as_plain_ram().unwrap().to_scalar::<i32>().unwrap();
        ...
        let k_scale = if self.qp[3].len() == 1 {
            ...
            self.qp[3].try_as_plain_ram().unwrap().as_slice::<f32>().unwrap().into()
        };
    ...
    fn output_dt(&self) -> DatumType {
        self.raw_output_dt.quantize(QParams::ZpScale {
            ...
            scale: *self.qp[5].try_as_plain_ram().unwrap().to_scalar().unwrap(),
        })
    }
    ...
    fn tract(&self) -> TractResult<TypedModel> {
        ...
        let idt = self.data.datum_type().quantize(QParams::ZpScale {
            zero_point: self.qp[0].cast_to::<i32>()?.try_as_plain_ram()?.as_slice::<i32>()?[0],
            scale: self.qp[1].cast_to::<f32>()?.try_as_plain_ram()?.as_slice::<f32>()?[0],
        });
        let kdt = self.kernel.datum_type().quantize(QParams::ZpScale {
            zero_point: self.qp[2].cast_to::<i32>()?.try_as_plain_ram()?.as_slice::<i32>()?[0],
            scale: self.qp[3].cast_to::<f32>()?.try_as_plain_ram()?.as_slice::<f32>()?[0],
        });
...
impl Test for QConvProblem {
    fn run_with_approx(
        ...
        let idt = self.data.datum_type().quantize(QParams::ZpScale {
            ...
            scale: *self.qp[1].try_as_plain_ram()?.to_scalar()?,
        });
...
pub fn suite() -> TractResult<TestSuite> {
    ...
    *kernel.try_as_plain_ram_mut().unwrap().as_slice_mut::<i8>().unwrap().last_mut().unwrap() = -1;
    suite.add(
        "bias_3",
        QConvProblem {
            shape_in: HWC.from_n_c_hw(1, 1, [2]).unwrap(),
