...
/// aliases whenever the innermost op does. Without this the memory schema
/// treats the alias source as dead after the wrapper's step and recycles
/// its arena region under a live view.
pub fn fused_axis_op_may_alias(node: &TypedNode) -> bool {
    fn inner_aliases(op: &dyn TypedOp) -> bool {
        if let Some(w) = op.downcast_ref::<MetalFusedAxisOp>() {
    ...
    node.op_as::<MetalFusedAxisOp>().is_some_and(|w| inner_aliases(w.op.as_ref()))
}
