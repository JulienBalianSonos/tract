...
/// Cached flag lookup: several call sites run per decode step (getenv on the
/// hot path measurably shows up in decode CPU profiles), and none of these
/// flags is toggled mid-process.
fn env_flag_with_legacy(new: &'static str, legacy: &'static str) -> bool {
...
impl OpState for MetalFusedSdpaState {
    fn eval(
        ...
        let use_flash = s_len <= FLASH_MAX_S
            && t_len >= flash_min_t()
            && d <= 64
            // The kernel's QK dot is half4-vectorized with no scalar tail.
            && d % 4 == 0
            && group <= 8
...
impl DeviceKvBuffer {
    ...
    fn deep_copy(&self) -> TractResult<Self> {
        ...
        if self.buf.is_some() && self.len > 0 {
            ...
            let strides = self.logical_strides(self.cap);
            let dst = DeviceTensor::ArenaView(DeviceArenaView::from_owned(
                fresh.clone(),
                f16::datum_type(),
                tvec![1, self.hkv, self.len, self.d],
                strides.clone(),
