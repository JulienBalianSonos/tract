...
pub(crate) fn maybe_probe(plan: &Arc<TypedSimplePlan>) {
    ...
    let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| probe(plan, resolved)));
    let winner = match outcome {
        ...
        Err(_) => {
            log::warn!("Metal load-time autotune probe panicked; keeping the resolved profile");
            resolved
        }
    };
...
fn plan_measure_runs(step: Duration, remaining: Duration, candidates: usize) -> Option<usize> {
    let total_runs = |runs: usize| 2 + runs + candidates * (1 + runs) + 1;
    MEASURE_RUNS_LADDER
        .iter()
        .copied()
...
fn synthesize_inputs(model: &TypedModel) -> Result<TVec<TValue>, String> {
    ...
    for (ix, outlet) in inputs.iter().enumerate() {
        let fact =
            model.outlet_fact(*outlet).map_err(|e| format!("no fact for input #{ix}: {e}"))?;
...
mod tests {
    ...
    fn synthesize_decode_shaped_zero_inputs() {
        ...
        let ids = model.add_source("input_ids", i64::fact(&[s.to_dim()])).unwrap();
        let kv =
            model.add_source("kv", f32::fact(&[1.into(), 4.into(), p.to_dim(), 8.into()])).unwrap();
        ...
        assert!(
            inputs[1].try_as_plain().unwrap().as_slice::<f32>().unwrap().iter().all(|&v| v == 0.0)
        );
    }
    ...
    fn synthesize_declines_non_copy_dtypes() {
        let mut model = TypedModel::default();
        let x = model.add_source("x", TypedFact::dt_shape(DatumType::TDim, [1usize, 2])).unwrap();
